...
static void EvlDyadicStrFun(int fun)
{
	...
	char *pchrL, *pchrR;
	double *pnew;
	int nElem, nElem2, typL, typR;

	typL = TYPE(poprTop);
	typR = TYPE(SECOND(poprTop));

	if (typL == TCHR) {	// CHR op XXX
		chrL = VCHR(poprTop);
		POP(poprTop);
		if (typR == TCHR) {	// CHR op CHR
			TYPE(poprTop) = TNUM;
			chrR = VCHR(poprTop);
chr_chr:	switch (fun) {
			case APL_EQUAL:
				VNUM(poprTop) = chrL == chrR;
				break;
			case APL_NOT_EQUAL:
				VNUM(poprTop) = chrL != chrR;
				break;
			...
			pchrR = VPTR(poprTop);
chr_str:	pnew  = TempAlloc(sizeof(double), nElem);
			TYPE(poprTop) = TARR;
			VOFF(poprTop) = WKSOFF(pnew);
			switch (fun) {
			case APL_EQUAL:
		...
		POP(poprTop);
		if (typR == TCHR) {	// STR op CHR
			chrR = VCHR(poprTop);
			// Copy rank and shape from left argument
str_chr:	*poprTop = *(poprTop - 1);
			TYPE(poprTop) = TARR;
			VOFF(poprTop) = WKSOFF(pnew);
			switch (fun) {
			case APL_EQUAL:
			...
			pchrR = VPTR(poprTop);
			// Treat single element string as a character
			if ((nElem2 = NumElem(poprTop)) == 1) {
				chrR = *pchrR;
				if (nElem == 1) {
					chrL = *pchrL;
					pnew  = TempAlloc(sizeof(double), nElem);
					// If ranks differ, adopt the higher one
					if (RANK(poprTop - 1) > RANK(poprTop))
						*poprTop = *(poprTop - 1);
					TYPE(poprTop) = TARR;
					VOFF(poprTop) = WKSOFF(pnew);
					goto chr_chr;				
			...
			if (!Conformable(poprTop, ABOVE(poprTop)))
				EvlError(EE_NOT_CONFORMABLE);
			VOFF(poprTop) = WKSOFF(pnew);
			TYPE(poprTop) = TARR;
			switch (fun) {
...
#if	0
	double numL, numR;
	double *pnumL, *pnumR, *pnew;
	int nElem, nElem2, typL, typR;

	typL = TYPE(poprTop);
	typR = TYPE(SECOND(poprTop));

	if (typL == TNUM) {	/* NUM op XXX */
		numL = VNUM(poprTop);
		POP(poprTop);
		if (typR == TNUM) {		/* NUM op NUM */
			numR = VNUM(poprTop);
num_num:	VNUM(poprTop) = EvlDyadicScalarNumFun(fun, numL, numR);
		} else {				/* NUM op ARR */
			nElem = NumElem(poprTop);
			pnumR  = VPTR(poprTop);
num_arr:	pnew  = TempAlloc(sizeof(double), nElem);
			VOFF(poprTop) = WKSOFF(pnew);
			switch (fun) {
			case APL_CIRCLE:
				while (nElem--) {
					*pnew++ = EvlCircularFun((int)numL, *pnumR++);
				}
				break;
			case APL_UP_STILE:
				while (nElem--) {
		...
		pnew  = TempAlloc(sizeof(double), nElem);
		POP(poprTop);
		if (typR == TNUM) {	/* ARR op NUM */
			numR = VNUM(poprTop);
arr_num:	*poprTop = *(ABOVE(poprTop)); /* Copy descriptor */
			VOFF(poprTop) = WKSOFF(pnew);
			switch (fun) {
			case APL_CIRCLE:
				while (nElem--) {
					*pnew++ = EvlCircularFun((int)*pnumL++, numR);
				}
				break;
			case APL_UP_STILE:
				while (nElem--) {
			...
			pnumR = VPTR(poprTop);
			// Treat single element array as a number
			if ((nElem2 = NumElem(poprTop)) == 1) {
				numR = *pnumR;
				if (nElem == 1) {
					numL = *pnumL;
			...
			case APL_CIRCLE:
				while (nElem--) {
					numL = *pnumL++;
					numR = *pnumR++;
					*pnew++ = EvlCircularFun((int)numL, numR);
				}
				break;
			case APL_UP_STILE:
				while (nElem--) {
